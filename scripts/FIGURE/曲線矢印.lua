...
local l = math.floor(length) / 2
local size = track_size / math.sqrt(2)
NN = track2 - 1
...
local h = 0
for i = 0, NN do
    local dx = math.ceil(math.abs(pos[i * 2 + 1]) + l + size + 1)
    local dy = math.ceil(math.abs(pos[i * 2 + 2]) + l + size + 1)
    if w < dx then
        w = dx
    end
    if h < dy then
...
local sx = {}
local sy = {}
local sx1, sy1 = PassXYZ(0 / (NN * BN))
local sx2, sy2 = PassXYZ(1 / (NN * BN))
local r = math.atan2(sy1 - sy2, sx2 - sx1)
for i = 1, 4 do
