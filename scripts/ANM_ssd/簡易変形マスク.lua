...
---$script_tips:マスクが簡易変形します。本家「簡易変形」を改変したもの。
---$tips:直線移動等にすると、終点のアンカーポイントが表示され、時間経過で変形できるようになります。
---:0→100に設定すれば、それに応じて時間が経過します。
---$track:時間経過
---min=0
---max=100
local elapsed = 0
---$tips:マスクの輪郭が明確になります。図形を円にした場合などに分かりやすいです。
---$track:サイズ
---min=0
---max=2000
local fsize = 100
...
---$tips:アンカーポイントの座標を指定します。通常はドラッグして調整するので、触らなくてもいいかも。
---$value:座標
local pos = { -100, -100, 100, -100, 100, 100, -100, 100 }

---$tips:マスクの輪郭のなめらかさを指定します。
---$track:分割数
---min=1
---max=50
---step=1
