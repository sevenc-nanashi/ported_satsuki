--label:${ROOT_CATEGORY}\切り替え効果\@TA
---$script_tips:テキストが、縦長or横長になってぼやけて登場してきます。
---$tips:登場時間を指定します。マイナスで退場します。
---$track:時間[s]
---min=-5
---max=5
---step=0.01
local duration = 0.3
---$tips:文字の登場間隔を指定します。
---$track:間隔[s]
---min=0
---max=5
---step=0.01
local interval = 0.3
---$tips:ぼかしの強さ(範囲)を指定します。
---$track:範囲
---min=0
---max=300
---step=1
local blur_range = 100
---$tips:登場する順番を指定します。
---$select:登場順
---順番=0
---逆順=1
---ランダム順=2
---ランダム間隔=3
---内側から=4
---外側から=5
local order = 0
---$tips:ぼかしの方向が横方向になります。
---$check:横方向
local horizontal = false

---$tips:登場してくる拡大率を指定します。
---$track:拡大率
---min=1
---max=1000
---step=0.001
local zoom_rate = 400

local aspect_direction = 1

local order_index
if order == 0 then
    order_index = obj.index -- 順番に登場
elseif order == 1 then
    order_index = obj.num - 1 - obj.index -- 後ろから登場
elseif order == 2 then
    local indexes = {}
    for i = 0, obj.num - 1 do
        indexes[i + 1] = i
    end
    for i = 0, obj.num - 1 do
        local dest = rand(0, obj.num - 1, -obj.num, i + 1)
        local swap = indexes[i + 1]
        indexes[i + 1] = indexes[dest + 1]
        indexes[dest + 1] = swap
    end
    order_index = indexes[obj.index + 1] -- ランダム順に登場
elseif order == 3 then
    order_index = rand(0, 100 * (obj.num - 1), obj.index, 0) / 100 -- ランダム間隔に登場
elseif order == 4 then
    order_index = math.abs((obj.num - 1) / 2 - obj.index) -- 内側から登場
else
    order_index = (obj.num - 1) / 2 - math.abs((obj.num - 1) / 2 - obj.index) -- 外側から登場
end

local progress
if duration < 0 then
    progress = (duration - obj.num * interval - obj.time + obj.totaltime + order_index * interval) / duration
else
    progress = (duration - obj.time + order_index * interval) / duration
end

if progress > 0 then
    if progress > 1 then
        obj.alpha = 0
        progress = 1
    end

    progress = progress * progress
    if horizontal then
        aspect_direction = -aspect_direction
    end
    obj.zoom = obj.zoom + progress * (zoom_rate - 100) / 100
    obj.aspect = obj.aspect + progress * (1 - 1 / zoom_rate * 100) * aspect_direction
    obj.effect("ぼかし", "範囲", progress * blur_range, "縦横比", -100 * aspect_direction)
end
