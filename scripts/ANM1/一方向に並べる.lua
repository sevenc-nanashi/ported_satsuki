---$script_tips:同じ画像を一方向に増やしていきます。
---:画像ループの一種のようなものです。図形などに適用するとよいかもしれません。
--label:${ROOT_CATEGORY}\配置
---$tips:画像の個数です。
---:トラックバーで直線移動させることで、オブジェクトが一方向に増えていくようにすることができます。
---$track:個数
---min=1
---max=200
---step=1
local n = 5
---$tips:画像間の間隔を指定します。数値は距離(ピクセル)です。
---$track:間隔
---min=-1000
---max=1000
local l = 50
...
---max=720
local angle = 0
---$tips:度の平面軸に沿って回転し、伸びていくか指定します。
---$select:回転軸
---X軸=0
---Y軸=1
---Z軸=2
local rotation_axis = 0
---$tips:オブジェクトが双方向に増えていきます。
---$check:双方向
